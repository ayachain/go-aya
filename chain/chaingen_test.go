package chain

import (
	"context"
	"fmt"
	"github.com/ayachain/go-aya/consensus/impls/APOS"
	Avdb "github.com/ayachain/go-aya/vdb"
	"github.com/ayachain/go-aya/vdb/block"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ipfs/go-datastore"
	syncds "github.com/ipfs/go-datastore/sync"
	"github.com/ipfs/go-ipfs-config"
	ipfscore "github.com/ipfs/go-ipfs/core"
	"github.com/ipfs/go-ipfs/repo"
	"testing"
)

var TestAddr = common.HexToAddress("0xD2bfC9AC49F3F0CfC1cBDF1cf579593D6De85435")

var ChainGenJson = `
{
	"index": 0,
	"chainid": "aya",
	"parent": "f919678e3089f1da5b4eac138c86efa0461555223f71f0d53a5a1ea9472834a0",
	"timestamp": 1501516800,
	"append": "SSUyMHJlc2VydmVkJTIwYW4lMjBpbXBvcnRhbnQlMjBtZXNzYWdlJTIwaW4lMjBwYXJlbnQlMjBoYXNoLg==",
	"extradata": "",
	"txc": 0,
	"txs": "",
	"award": {
		"0xD2bfC9AC49F3F0CfC1cBDF1cf579593D6De85435": 1000000000
	}
}
`
var testIdentity = config.Identity{
	PeerID:  "QmNgdzLieYi8tgfo2WfTUzNVH5hQK9oAYGVf6dxN12NrHt",
	PrivKey: "",
}

var MockNode = make([]*ipfscore.IpfsNode, 2)

func TestInitialization(t *testing.T) {

	ctx := context.Background()
	id := testIdentity

	good := []*config.Config{
		{
			Identity: id,
			Addresses: config.Addresses{
				Swarm: []string{"/ip4/0.0.0.0/tcp/4001"},
				API:   []string{"/ip4/127.0.0.1/tcp/8000"},
			},
		},

		{
			Identity: id,
			Addresses: config.Addresses{
				Swarm: []string{"/ip4/0.0.0.0/tcp/4001"},
				API:   []string{"/ip4/127.0.0.1/tcp/8000"},
			},
		},
	}

	bad := []*config.Config{
		{},
	}

	for i, c := range good {
		r := &repo.Mock{
			C: *c,
			D: syncds.MutexWrap(datastore.NewMapDatastore()),
		}
		n, err := ipfscore.NewNode(ctx, &ipfscore.BuildCfg{Repo: r})
		if n == nil || err != nil {
			t.Error("Should have constructed.", i, err)
		}

		MockNode[i] = n
	}

	for i, c := range bad {
		r := &repo.Mock{
			C: *c,
			D: syncds.MutexWrap(datastore.NewMapDatastore()),
		}
		n, err := ipfscore.NewNode(ctx, &ipfscore.BuildCfg{Repo: r})
		if n != nil || err == nil {
			t.Error("Should have failed to construct.", i)
		}
	}
}


var TestMainVDB Avdb.CVFS

func TestGenChaiDB(t *testing.T) {

	genBlock := &block.GenBlock{}

	if err := genBlock.Decode([]byte(ChainGenJson)); err != nil {
		t.Fatal("gen block unmarshal expected")
	}

	TestMainVDB, err := Avdb.CreateVFS( genBlock, MockNode[0])
	if err != nil {
		t.Fatalf("CreateVFSFailed : %v", err )
	}

	asset, err := TestMainVDB.Assetses().AssetsOf( TestAddr.Bytes() )
	if err != nil {
		t.Fatalf("AssetsOf : %v", err )
	}

	fmt.Printf("Address:%v\tAvail:%v\tVote:%v\tLocked:%v\n", TestAddr.String(), asset.Avail, asset.Vote, asset.Locked)

}

var APOSNotary *APOS.APOSConsensusNotary

func TestAPOSStarting (t *testing.T) {

	APOSNotary := APOS.NewAPOSConsensusNotary( TestMainVDB, MockNode[0] )

	if APOSNotary == nil {
		t.Fatal( "NewAPOSConsensusNotary expected." )
		t.FailNow()
	}

	APOSNotary.StartWorking()
	fmt.Println("APOSNotary is started work")

}


//func TestSendTx (t *testing.T ) {
//
//	err := APOSNotary.HiDog().TakeMessage( &pubsub.Message{
//
//	})
//
//	if err != nil {
//		t.Fatalf("TakeMessage error")
//	}
//
//}
//
